...
# (column-name field, key into a resolution_at_charge result dict, console-table header label)
...
#  Resolution fields shown in the console table by default (verbose=False); the rest
#  (sigma_e_err, reduced_chi2, delta_aic, n_peaks_fit, expected_peaks) are only shown
#  with verbose=True. The saved CSV is unaffected either way -- it always has every column.
